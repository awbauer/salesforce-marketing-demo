...
- [x] Per-user OAuth connection, reconnect, expired-auth, permission-denied, and recovery states are implemented without browser-stored provider tokens.
...
- Screenshots: `artifacts/evidence/WU-003/confirmation-{chrome,edge}.png`, `workbench-{chrome,edge}.png`, and `oauth-recovery-{chrome,edge}.png` capture the explicit confirmation, durable read-back, and expired-authorization recovery journeys. Both Playwright projects passed all four journeys on 2026-09-24.
- Local verification: the latest unrestricted `pnpm verify` run passed all 11 local gates, including 14 unit tests, 11 Worker tests, 20/20 routing evaluations, the Salesforce metadata contract, and the production build. `pnpm test:e2e` passed 4/4 Chrome/Edge journeys, including an actionable expired-OAuth recovery state. `SF_TARGET_ORG=northstar-pot SF_APPROVED_PROOF_ORG_ID=<protected-value> pnpm sf:validate` proved the exact approved org, Campaign and Account samples, active business unit and data space, queryable consent surfaces, 506 identity links, and Agent Script availability, then correctly exited nonzero because all checked consent stores contain zero representative records. `sf project convert source --root-dir salesforce/force-app --output-dir /tmp/northstar-phase2-mdapi --json` completed successfully as an offline Metadata API conversion check.
...
- Cloudflare deployment: the checked-in D1 confirmation-audit migration is applied and read back. Worker version `d162cbcc-4c5d-40d9-b550-0d671834d357` (v19), deployed on 2026-09-24, preserves the portal URL, three secret bindings, D1, R2, AI, Durable Object, and assets bindings. It provides explicit OAuth recovery states, migrates persisted proof sessions from the retired placeholder to Campaign `701jV000004GglIQAS`, reports only the governed 13-tool catalog, and returns structured failures for rejected upstream writes. An unauthenticated `/api/session` read-back returned the expected Access HTTP 302. Evaluator PKCE OAuth completed through the Worker and the live UI read back the exact 13 curated tools. After an explicit five-minute confirmation, Salesforce created review Task `00TjV000000uDyfUAE`; D1 recorded confirmation `09e6fb60-5020-4b7b-8d43-32d951c6de5a` as executed with idempotency key `49ee5878-bd4f-4b6a-b09d-83d4390ba485`, and Salesforce read-back returned the same key, Campaign `701jV000004GglIQAS`, status `Not Started`, and subject `Northstar campaign readiness review`. The reusable `AWB` policy remains unchanged. A prior fabricated-confirmation negative test created Task `00TjV000000pyVBUAY`, proving the former Apex presence-only checks were insufficient; that Task was read back and not deleted. Both writes were disabled during remediation, then re-enabled only after the live five-field Apex contract matched Cloudflare's immutable manual-OAuth schema. Matching proof secrets are configured in Salesforce and the Worker without source-control exposure.
- Known limitations: `northstar-pot` is production-classified by Salesforce but is treated as the dedicated proof org by explicit user instruction and exact runtime org-ID binding; see ADR-003. Five CLI publish attempts timed out at the Agentforce authoring endpoint, but the approved Agentforce Builder path completed the custom agent lifecycle. Marketing Cloud, Data Cloud, deployed data streams, Agentforce, and generative AI are active. Northstar Campaign Creation, Northstar Content Builder, and Northstar Account Discovery are active and their generated Agent Script sources are versioned. The pre-existing Fizi agent is outside this proof and was not modified. The active Hosted MCP server, per-user Worker OAuth, cryptographic confirmation verifier, confirmation-audit schema, and live confirmed write/read-back are demonstrated. Representative consent data is absent; the proof will not manufacture consent records outside its allowed writes, so the consent portion of the sandbox smoke criterion remains open.
