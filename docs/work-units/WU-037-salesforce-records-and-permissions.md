...
- A live read-only save preflight on 2026-09-26 showed `check_write_access` succeeding in the Cloudflare portal, followed by a Worker 500 from production D1: the original confirmation-audit constraint accepted only the two Phase 2 actions. Migration `0003_expand_confirmation_actions.sql` rebuilds the table without losing audit rows and admits exactly the six governed writes; the contract gate now checks every allowed action against that migration.
