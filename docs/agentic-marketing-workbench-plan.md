...
- Use Cloudflare-hosted `@cf/openai/gpt-oss-20b` as the orchestrator model. It supports function calling and reasoning while keeping the proof on one platform and avoiding another provider credential. ADR-005 records why it returned from `@cf/openai/gpt-oss-120b` (ADR-004).
