...
- [ ] Connect the Salesforce server through the Cloudflare MCP portal with per-user OAuth and verify recovery states. Portal OAuth, synchronization, and the read path pass; the evaluator's Worker-connector OAuth and recovery-state proof remain.
- [ ] Run the representative-record smoke suite and the one confirmed write/read-back proof.
...
Current Cloudflare read-back: the confirmation-audit migration is applied and Worker v11 is deployed with the portal URL and a secret signing-key binding. The net-new upstream, portal, proxied CNAME, and both dedicated Access applications are live; `AWB` remains unchanged. Evaluator PKCE OAuth, initialize/initialized, server discovery, exact 13-tool synchronization, and a live readiness call passed. A fabricated-confirmation negative test then exposed that the original Apex actions accepted non-empty but unauthenticated confirmation fields and created proof Task `00TjV000000pyVBUAY`. Both writes were immediately disabled while HMAC verification was added. Because Cloudflare manual-OAuth schemas are immutable after first authorization, the remediated transport now carries the five-minute expiry, principal, and HMAC inside a versioned confirmation token while retaining the already-cached five-field tool schema. Salesforce deployment `0AfjV000002aQajSAE`, Apex run `707jV000003xCcvQAE`, and the refreshed direct protocol gate prove the compatible contract, 9/9 tests, exact 13-tool discovery, and rejection of an unconfirmed write. The two writes are re-enabled in the default-deny portal but remain unavailable to autonomous model execution; the evaluator's Worker-only confirmed path and authoritative read-back are still pending.
